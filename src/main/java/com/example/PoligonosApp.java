...
public class PoligonosApp extends Application {
    ...
    private Polygon criaPoligono(List<Point> listaPontos) {
        final var poligono = new Polygon();

        poligono.getPoints().addAll(
                listaPontos.stream()
                        .flatMap(ponto -> Stream.of(ponto.x(), ponto.y()))
                        .toList()
        );
    ...
    protected List<String> tipoPoligonos(){
        return pontosPoligonos.stream()
                .flatMap(lista ->
                        Stream.of(lista.size())
                        .map(this::nomePoligono))
                .toList();
    }
    ...
    protected List<Double> perimetros(){
        return pontosPoligonos.stream()
                .flatMap(poligono -> {
                    var ultimoPonto = poligono.getLast();
                    var perimetro = poligono.stream()
                                             .reduce(ultimoPonto, Point::new)
                                             .distance();
